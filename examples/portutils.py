...
class PortUtils(Util):
    ...
    def get_macaddrs(self, glob=None, keymacaddr=True):
        ...
        eths = rsp.xpath(
            'physical-interface[normalize-space(link-level-type)="Ethernet"]')

        def _m2n_():
            """ macaddr => port-name """
            return {
                port.findtext('current-physical-address').strip():
                port.findtext('name').strip()
                for port in eths
            }
    ...
    def find_macaddr_by_ipaddr(self, ipaddr, glob=None):
        ...
        found = rsp.xpath(
            './/ifa-local[starts-with(normalize-space(.),"%s")]' %
            ipaddr)
        if 0 == len(found):
            return None
        ...
        if found_ipaddr.split('/')[0] != ipaddr:
            return None
        ...
        if_name = found[0].xpath(
            'ancestor::physical-interface/name')[0].text.strip()

        # now that we've got the physical interface name, we can retrieve the
        # MAC address.

        rsp = self.rpc.get_interface_information(
